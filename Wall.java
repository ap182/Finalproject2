...
public class Wall {
	...
	int y = rnd.nextInt(Game.SIZE - 400) + 200;	//generates the y value that is the top of the bottom wall
	static int speed = - 4;							//scrolling speed
	int WIDTH = 45;									//width of a wall, it's a constant 
	int height = Game.SIZE - y;					//height of the wall, just the height of the window - how high the wall is
	...
	public void paint(Graphics g){
		...
		g.drawImage(img, x, ( -Game.SIZE ) + ( y - GAP), null);	//bottom part
	}
	...
	public void move(){
		...
		Rectangle wallBoundsTop = new Rectangle(x, 0, WIDTH, Game.SIZE - (height + GAP));
		
		//If Basketball collids with a wall, he dies and  the game, bird, and walls are all reset
		if ( (wallBounds.intersects(Basketball.getBounds()) ) || (wallBoundsTop.intersects(Basketball.getBounds()))){
			Basketball.reset();
			died();
		}
		...
		if (x <= 0 - WIDTH){
			x = Game.SIZE;
			y = rnd.nextInt(Game.SIZE - 400) + 200;
			height = Game.SIZE - y;
		}		
	}
