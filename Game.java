...
public class Game extends JPanel{
	...
	static int SIZE = 800;						//height of the window
												//width of the window
	Basketball ball = new Basketball();					//makes a new bird 
	Wall wall = new Wall(SIZE);					//makes the first wall you see
	Wall wall2 = new Wall(SIZE + (SIZE / 2));		//makes the second wall you see
	...
	try {
		img = ImageIO.read(new File("court.jpg"));
	} catch (IOException e) {
	...
	public void paint(Graphics g){
		...
		g.drawImage(img, scrollX + 1304, 0, null);			//number 2, exactly one background length away (1800 pixels)
		...
		wall2.paint(g);			//the second wall
 		...
 		g.drawString("" + score, SIZE / 2 - 20, 700);
	...
	public void move(){
		...
		ball.move();			//moves the wee little birdy
		...
		if ( (wall.x == Basketball.X) || (wall2.x == Basketball.X) ) 	//Increments the score when the player passes a wall
			score();
	}
